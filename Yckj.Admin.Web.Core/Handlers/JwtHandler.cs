...
using Yckj.Admin.Application;
using System.Collections.Generic;
using System.Linq;

namespace Yckj.Admin.Web.Core
{
    public class JwtHandler : AppAuthorizeHandler
    {
        ...
        private static async Task<bool> CheckAuthorizeAsync(DefaultHttpContext httpContext)
        {
            ...
            {
                var oAuthPermList = App.GetService<SysCacheService>().Get<List<SysOAuthPerm>>(CacheConst.KeyOAuthPermCache);
                if (oAuthPermList == null || oAuthPermList.Count == 0) oAuthPermList = await App.GetService<SysOAuthPermService>().GetList();
                var route = httpContext.Request.Path.ToString().ToLower();
                return oAuthPermList.Exists(it => it.Identity == (int)LoginModeEnum.APP && it.Route == route);
            }
            else
            {
                // 排除超管
